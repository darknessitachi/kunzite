...
public class DefaultPositionBook extends AbstractPositionBook {
    @Inject
    DefaultPositionBook(PositionFactory positionFactory, PositionUpdater positionUpdater,
    ...
    public Position getPositionFor(String portfolioId) {
        Map<String, Position> positionMap = getPositionByPortfolioMap();

        Position res = positionMap.get(portfolioId);

        if (res == null) {
            res = createPosition(portfolioId);
            positionMap.put(portfolioId, res);
        }
